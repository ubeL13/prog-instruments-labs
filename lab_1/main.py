...
async def worker_add_orders(message: types.Message):
    ...
    if 'накрутить' in message.text.lower():
        try:
            ...
            await bot.send_message(
                text=profile_text(message.from_user.id, count),
                chat_id=message.chat.id
            )
        except IndexError:
            pass
...
async def send_help(message: types.Message):
    ...
    help_text = (
        ...
        "Click the buttons in the menu to access other functions."
    )
...
async def saint_p_layout(call: types.CallbackQuery):
    ...
    await bot.answer_callback_query(call.id)
    await bot.edit_message_text(
        "Choose a district in Saint Petersburg:",
...
async def pay(call: types.CallbackQuery):
    ...
    Args:
        call (types.CallbackQuery): The callback query object containing the request data.
    """
    await bot.answer_callback_query(call.id)
    await bot.send_message(
        chat_id=call.message.chat.id,
